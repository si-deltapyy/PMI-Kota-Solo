        ...
        <div class="card">
          <div class="card-body">
            ...
            <div class="d-flex justify-content-between align-items-center mb-6 ms-auto">
                <!-- Add Filter Buttons -->
                <div>
                    <a href="{{ route('relawan-laporankejadian', ['status' => 'On Process'] && ['status' => 'Valid'] && ['status' => 'Invalid']) }}" class="btn btn-outline-primary btn-sm @if(empty(request('status'))) active @endif">All</a>
                    <a href="{{ route('relawan-laporankejadian', ['status' => 'On Process']) }}" class="btn btn-outline-warning btn-sm @if(request('status') == 'On Process') active @endif">Dalam Proses</a>
                    <a href="{{ route('relawan-laporankejadian', ['status' => 'Valid']) }}" class="btn btn-outline-success btn-sm @if(request('status') == 'Valid') active @endif">Valid</a>
                    <a href="{{ route('relawan-laporankejadian', ['status' => 'Invalid']) }}" class="btn btn-outline-danger btn-sm @if(request('status') == 'Invalid') active @endif">Invalid</a>
                </div>
             <!-- Add Search Form -->
            ...
            <div class="table-responsive pt-3">
              <table class="table table-bordered">
                ...
                <tbody>
                  ...
                  <div class="modal fade" id="approveModal{{ $report->id_report }}" tabindex="-1" aria-labelledby="exampleModalLabel{{ $report->id }}" aria-hidden="true">
